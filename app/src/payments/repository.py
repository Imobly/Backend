...
class PaymentRepository(BaseRepository[Payment, PaymentCreate, PaymentUpdate]):
    ...
    def get_overdue_payments(self, db: Session) -> List[Payment]:
        ...
        return (
            db.query(Payment)
            .filter(Payment.status.in_(["pending", "partial"]), Payment.due_date < date.today())
            .all()
        )
    ...
    def get_payments_by_period(
        self, db: Session, start_date: date, end_date: date, payment_field: str = "due_date"
    ) -> List[Payment]:
        """Buscar pagamentos por período"""
        if payment_field == "payment_date":
            return (
                db.query(Payment).filter(Payment.payment_date.between(start_date, end_date)).all()
            )
        else:
            return db.query(Payment).filter(Payment.due_date.between(start_date, end_date)).all()
