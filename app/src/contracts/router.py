...
async def list_contracts(
    skip: int = 0,
    limit: int = 100,
    status: Optional[str] = Query(None, description="Filtrar por status: active, expired, terminated"),
    ...
    return contract_controller(db).get_contracts(
        db, user_id, skip=skip, limit=limit, status=status, property_id=property_id, tenant_id=tenant_id
    )
...
async def update_contract_status(
    ...
    contract_obj = contract_controller(db).repository.update_status(db, contract_id, user_id, new_status)
    if not contract_obj:
