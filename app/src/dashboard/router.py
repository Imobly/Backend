...
async def get_revenue_vs_expenses(
    months: int = Query(default=12, ge=1, le=24),
    property_id: Optional[int] = Query(default=None, description="Filtrar por propriedade específica"),
    db: Session = Depends(get_db),
    user_id: int = Depends(get_current_user_id_from_token)
):
    """
...
async def get_financial_overview(
    ...
    if property_id:
        expense_categories_query = expense_categories_query.filter(Expense.property_id == property_id)
    ...
    return {
        ...
        "summary": {
            ...
            "profit_margin": round(((float(total_revenue) - float(total_expenses)) / max(float(total_revenue), 1) * 100), 2)
        },
        "expense_breakdown": [
